...
// [CHANGED] Removed app.use("/admin", adminSessionValidation) — it conflicts with
// individually-registered routes by stripping the /admin prefix mid-match.
// adminSessionValidation is now applied directly on each route instead.
...
app.post("/admin/employees/generate-code", adminSessionValidation, async (req, res) => {
  try {
    ...
    const newEmployee = new Employee({
      name: "Pending",
      email: `pending-${Date.now()}@foodbank.org`,
      pin,
      isActive: false,
    });
    await newEmployee.save();
    res.json({ employeeId: newEmployee.employeeId, pin });
  } catch (err) {
    console.error(err);
...
app.get("/admin/test", (req, res) => res.send("employee routes are loaded"));
